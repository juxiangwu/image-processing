        ...
        )
{
    ...
    std::cout <<"init_net:\n";
    std::cout << "net_inw:"<<net_inw << ",net_inh:"<<net_inh<<std::endl;
    std::cout << "net_outw:" <<net_outw <<",net_outh:"<<net_outh<<std::endl;
    // 3. resize to net input size, put scaled image on the top left
    float scale = 0.0f;
    Mat netim = create_netsize_im(im, net_inw, net_inh, &scale);
    std::cout <<"create_netsize_im\n";
    ...
    for (int i = 0; i < NET_OUT_CHANNELS; ++i)
    {
        ...
        std::cout << "resize net output\n";
        std::cout << "netout size:"<<netout.size()<<std::endl;
        Mat nmsin(net_inh, net_inw, CV_32F, heatmap + net_inh*net_inw*i);
        std::cout << "nmsin size:" <<nmsin.size() <<std::endl;
