        ...
        if(cn>0)
        {
            ...
            if(max_area > 1000)
            {
                ...
                for(int i = 1; i <= hullcount; i++ )
                {
                    ...
                    cvLine( src, pt0, pt, cv::Scalar( 255, 0, 0 ), 1, CV_AA, 0 );
                    pt0 = pt;
                }
                for( ; defects; defects = defects->h_next)
                {
                    ...
                    for(int i=0; i<nomdef; i++)
                    {
                        if(defectArray[i].depth > 40 )
                        {
                            ...
                            cvLine(src, *(defectArray[i].start), *(defectArray[i].depth_point),cv::Scalar(255,255,0),1, CV_AA, 0 );
                            cvCircle( src, *(defectArray[i].depth_point), 5, cv::Scalar(0,0,255), 2, 8,0);
                            cvCircle( src, *(defectArray[i].start), 5, cv::Scalar(0,255,0), 2, 8,0);
                            cvLine(src, *(defectArray[i].depth_point), *(defectArray[i].end),cv::Scalar(0,255,255),1, CV_AA, 0 );
                            cvDrawContours(src,defects,cv::Scalar(0,0,0),cv::Scalar(255,0,0),-1,CV_FILLED,8);
                        }
                    }
                    // cout<<con<<"\n";
                    char txt[40]="";
