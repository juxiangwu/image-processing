...
__kernel void brightness_matrix_filter(__read_only image2d_t input,__write_only image2d_t output){
    const sampler_t sampler = CLK_FILTER_NEAREST |
                               CLK_NORMALIZED_COORDS_FALSE |
                               CLK_ADDRESS_CLAMP_TO_EDGE;
    ...
    float color_matrix[25] = {
        ...
        brightness / 255.0f, brightness / 255.0f, brightness / 255.0f,0.0f,1.0f
    };
    
    filter2d_internal(input,output,5,5,color_matrix,1);
