...
__kernel void video_effect_dots_filter(__read_only image2d_t input,__write_only image2d_t output){
   ...
   int sPattern_Dots[75] = {0, 1, 2, 0, 0,
                            1, 1, 1, 2, 0,
                            0, 1, 2, 2, 2,
                            0, 0, 1, 2, 0,
                            0, 1, 1, 1, 2,
    ...
    int pattern_width  = 5;
    int pattern_height = 15;
    
    float3 src_rgb = read_imagef(input,sampler,coord).xyz * 255.0f;
    float3 dst_rgb = src_rgb;
    
    int index =  sPattern_Dots[pattern_width * (coord.y % pattern_height) + (coord.x % pattern_width)];
