...
using namespace std;
#define WINDOW_NAME	"CUDA createSeparableLinearFilter"
...
void wait_for_a_while(uv_idle_t* handle){
    ...
    cvui::trackbar(frame, 20, 90, 180, &ksize, (int)1, (int)15);

    if (cvui::button(frame, 20, 160,180,30, "createSeparableLinearFilter")) {
        cv::Mat gray;
        std::vector<cv::Mat>channels;
        std::vector<cv::Mat>dst_channels;
        cv::split(src1,channels);
        int kernel_size = 1 *2 + 1;
...
//            int kz = ksize * 2 + 1;
            cv::Ptr<cv::cuda::Filter> filter =
                    cv::cuda::createSeparableLinearFilter(gray.type(),gray.type(),kernel,kernel);
            cv::Mat res;
