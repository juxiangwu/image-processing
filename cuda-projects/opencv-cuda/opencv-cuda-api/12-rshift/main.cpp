...
void wait_for_a_while(uv_idle_t* handle){
    ...
    if (cvui::button(frame, 15, 50,100,30, "rshift")) {
          cv::cuda::rshift(src1,cv::Scalar_<int>(4),dst);
    }

    if (cvui::button(frame, 15, 90,100,30, "lshift")) {
        cv::cuda::lshift(src1,cv::Scalar_<int>(4),dst);
    }
