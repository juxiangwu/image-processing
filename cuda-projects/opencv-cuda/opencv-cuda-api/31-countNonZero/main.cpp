...
using namespace std;
#define WINDOW_NAME	"CUDA countNonZero"
...
void wait_for_a_while(uv_idle_t* handle){
    ...
    if (cvui::button(frame, 15, 50,100,30, "countNonZero")) {
        cv::Mat gray;
        cv::cvtColor(src1,gray,cv::COLOR_BGR2GRAY);
        int res = cv::cuda::countNonZero(gray);
        std::cout << "res:" << res << std::endl;
    }

    if (cvui::button(frame, 15, 90,100,30, "countNonZero")) {
        cv::Mat res;
        cv::Mat gray;
        cv::cvtColor(src1,gray,cv::COLOR_BGR2GRAY);
        cv::cuda::countNonZero(gray,res);
        std::cout << "res:" << res <<std::endl;
    }
