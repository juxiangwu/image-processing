...
using namespace std;
#define WINDOW_NAME	"CUDA createLaplacianFilter"
...
void wait_for_a_while(uv_idle_t* handle){
    ...
    cvui::trackbar(frame, 20, 90, 180, &ksize, (int)1, (int)3);

    if (cvui::button(frame, 20, 160,180,30, "createLaplacianFilter")) {
        cv::Mat gray;
...
//            int kz = ksize * 2 + 1;
            cv::Ptr<cv::cuda::Filter> filter =
                    cv::cuda::createLaplacianFilter(gray.type(),gray.type(),ksize);
            cv::Mat res;
