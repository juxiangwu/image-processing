...
using namespace std;
#define WINDOW_NAME	"CUDA rectStdDev"
...
//    src2_rz.copyTo(roi2);
    ...
    if (cvui::button(frame, 15, 50,100,30, "rectStdDev")) {
        cv::Mat gray;
        cv::Mat res;
        cv::cvtColor(src1,gray,cv::COLOR_BGR2GRAY);
        gray.convertTo(gray,CV_32SC1);
        res = cv::Mat(gray.size(),gray.type());
        cv::Rect rect(0,0,gray.cols,gray.rows);
        cv::cuda::rectStdDev(src1,gray*gray,res,rect);
//        std::cout << "res:" << norm_val << std::endl;
