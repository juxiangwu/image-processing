...
using namespace std;
#define WINDOW_NAME	"CUDA createLinearFilter"
...
void wait_for_a_while(uv_idle_t* handle){
    ...
    if (cvui::button(frame, 20, 160,180,30, "createLinearFilter")) {
        cv::Mat gray;
        std::vector<cv::Mat>channels;
        std::vector<cv::Mat>dst_channels;
        cv::split(src1,channels);
        int kernel_size = ksize *2 + 1;
...
//            int kz = ksize * 2 + 1;
            cv::Ptr<cv::cuda::Filter> filter =
                    cv::cuda::createLinearFilter(gray.type(),gray.type(),kernel);
            cv::Mat res;
