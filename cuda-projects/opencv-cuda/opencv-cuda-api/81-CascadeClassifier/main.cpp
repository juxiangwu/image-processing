...
using namespace std;
#define WINDOW_NAME	"CUDA CascadeClassifier"
...
int offset = 10;
cv::Mat dst_rz;
cv::Ptr<cv::cuda::CascadeClassifier> classifier;
...
void wait_for_a_while(uv_idle_t* handle){
    ...
    if (cvui::button(frame, 20, 230,180,30, "CascadeClassifier")) {

        cv::Mat gray;
        cv::cvtColor(src1,gray,cv::COLOR_BGR2GRAY);
//        cv::threshold(gray,gray,180,240,cv::THRESH_BINARY);
        classifier = cv::cuda::CascadeClassifier::create("D:/Develop/DL/projects/digital-image-processing/datas/cvdata/haarcascades_cuda/haarcascade_frontalface_default.xml");

        std::cout << "create detector\n";

        cv::cuda::GpuMat dev_src(gray),dev_dst;
        classifier->detectMultiScale(dev_src,dev_dst);
        std::vector<cv::Rect> faces;
        classifier->convert(dev_dst,faces);
        src1.copyTo(dst);
        for(int i = 0; i < faces.size(); ++i)
           cv::rectangle(dst, faces[i], cv::Scalar(255,0,0));
...
int main()
{
    ...
    src1 = cv::imread("D:/Develop/DL/projects/digital-image-processing/datas/face.jpg");
    cv::resize(src1,src1,cv::Size(800,600));
