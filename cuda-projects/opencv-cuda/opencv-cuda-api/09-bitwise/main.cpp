...
void wait_for_a_while(uv_idle_t* handle){
    ...
    if (cvui::button(frame, 15, 50,100,30, "Bitwise And")) {
        cv::cuda::bitwise_and(src1,src2,dst);
    }

    if (cvui::button(frame, 15, 90,100,30, "Bitwise Or")) {
        cv::cuda::bitwise_or(src1,src2,dst);
    }

    if (cvui::button(frame, 15, 130,100,30, "Bitwise Not")) {
        cv::cuda::bitwise_not(src1,dst);
    }

    if (cvui::button(frame, 15, 170,100,30, "Bitwise XOR")) {
        cv::cuda::bitwise_and(src1,src2,dst);
    }
