...
using namespace std;
#define WINDOW_NAME	"CUDA findMinMax"
...
//    src2_rz.copyTo(roi2);
    ...
    if (cvui::button(frame, 15, 50,100,30, "findMinMax")) {
        cv::Mat gray;
        cv::Mat res;
        cv::cvtColor(src1,gray,cv::COLOR_BGR2GRAY);
        cv::cuda::findMinMax(gray,res);
        std::cout << res << std::endl;

    }

    if (cvui::button(frame, 15, 90,100,30, "findMinMaxLoc")) {
        cv::Mat minMax,loc;
        cv::Mat gray;
        cv::cvtColor(src1,gray,cv::COLOR_BGR2GRAY);
        cv::cuda::findMinMaxLoc(gray,minMax,loc);
        std::cout << "min max:" << minMax << ",loc:" << loc << std::endl;
    }
