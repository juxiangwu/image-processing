...
using namespace std;
#define WINDOW_NAME	"CUDA meanShiftFiltering"
...
void wait_for_a_while(uv_idle_t* handle){
    ...
    if (cvui::button(frame, 20, 230,180,30, "meanShiftFiltering")) {
        cv::Mat src_rgba;
        cv::cvtColor(src1,src_rgba,cv::COLOR_BGR2BGRA);
        cv::cuda::GpuMat dev_src(src_rgba),dev_dst;
        cv::cuda::meanShiftFiltering(dev_src,dev_dst,sp,sr);
        cv::Mat res;
        dev_dst.download(res);
        cv::cvtColor(res,res,cv::COLOR_BGRA2BGR);
        res.copyTo(dst);
