...
using namespace std;
#define WINDOW_NAME	"CUDA sqrIntegral"
...
//    src2_rz.copyTo(roi2);
    ...
    if (cvui::button(frame, 15, 50,100,30, "sqrIntegral")) {
        cv::Mat gray;
        cv::Mat res;
        cv::cvtColor(src1,gray,cv::COLOR_BGR2GRAY);
        cv::cuda::sqrIntegral(gray,res);
//        std::cout << "res:" << res << std::endl;
    ...
    if (cvui::button(frame, 15, 90,100,30, "sqrSum")) {
        cv::Scalar sum_val = cv::cuda::sum(src1);
        std::cout << "sqr sum:" << sum_val << std::endl;
    }

    if (cvui::button(frame, 15, 130,100,30, "sum")) {
        cv::Scalar sum_val = cv::cuda::sum(src1);
