...
using namespace std;
#define WINDOW_NAME	"CUDA gammaCorrection"
...
void wait_for_a_while(uv_idle_t* handle){
    ...
    if (cvui::button(frame, 20, 230,180,30, "gammaCorrection")) {
        cv::Mat rgba;
        cv::cvtColor(src1,rgba,cv::COLOR_BGR2BGRA);
        cv::cuda::GpuMat dev_src(rgba);
        const int order[4] = {0,2,1,3};
        cv::cuda::swapChannels(dev_src,order);
        dev_src.download(dst);
