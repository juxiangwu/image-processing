...
using namespace std;
#define WINDOW_NAME	"CUDA Bitwise"
...
void wait_for_a_while(uv_idle_t* handle){
    ...
    if (cvui::button(frame, 15, 50,100,30, "compare CMP_EQ")) {
        cv::cuda::compare(src1,src2,dst,cv::CMP_EQ);
    }

    if (cvui::button(frame, 15, 90,100,30, "compare CMP_GE")) {
         cv::cuda::compare(src1,src2,dst,cv::CMP_GE);
    }

    if (cvui::button(frame, 15, 130,100,30, "compare CMP_NE")) {
        cv::cuda::compare(src1,src2,dst,cv::CMP_NE);
    }

    if (cvui::button(frame, 15, 170,100,30, "compare CMP_LE")) {
        cv::cuda::compare(src1,src2,dst,cv::CMP_LE);
    }
